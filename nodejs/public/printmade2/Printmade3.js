...
// 라이센스 코드
var LICENSE_CODE = 'hKWl1Ms41Eygk91ToLN4VFzTFH7Q60rPfvRS20R28po=';
...
var browser = PTMChecker.checkWebBrowser();
	PTMBridge = { // 프린트메이드를 실행시키는 객체
		...
		Run : function(option){
			...
			var iframeid = "";
			PTM_LoadDialog();
			setTimeout(PTMBridge.timeingRun,1000);
...
function PTMPrintWebPage(options)
{
	...
	//if(	PTMInstallChecker.checkWebBrowser() == "MSIE"	){
		Donut.checkmode = 1;
...
/*         if (navigator.appVersion.indexOf("Linux")!=-1) this.isLinux = true; */
	...
	CallDOMParser : function(){
		...
		domparser.Run(frame,GPrintmade);
			...
			if(GPrintmade.isMac){
				...
				if(GPrintmade.isFirefox3 == true || GPrintmade.isFirefoxOver3 == true  || GPrintmade.isSafari == true || GPrintmade.isSafariOver51 == true){
					...
					//if(isPTM != -1){
						...
						//param = "I!FRAMEID:"+frameid+"|ELEMENTID:"+elementid+"|ALIGNMENT:left|CUTRECT:0,0,0,0|FORMURL:"+resultformurl+"|PAGEWIDTH:"+pagewidth+"|RENDERINGTYPE:"+RENDERINGTYPE;
						//alert(param);
