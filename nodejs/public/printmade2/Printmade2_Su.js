...
// 라이센스 코드
//var LICENSE_CODE = '';
var LICENSE_CODE = 'aHkPURpuoKR2Bkeb4DPGVynwE6UaSMpHYpGEGJ75DNdVJ+OJ3ihP4bwFHQBi0bqD';
// 공통 경로
//var SERVER_PATH = 'http://223.1.1.69:8280/printMade';
var SERVER_PATH = '/printMade';
//var SCRIPT_PATH = 'http://localhost/suhyub/Printmade2.js';
var SCRIPT_PATH = location.protocol + '//' + location.hostname + '/printMade/server_files/Printmade2.js';
...
var MAC_DOWNLOAD_URL = SERVER_PATH + '/download_files/Mac/Printmade2forMac.dmg'
...
//var FORMURL = location.protocol + '//' + location.hostname + '/printMade/server_files/form.txt';
var FORMURL = location.protocol + '//' + location.hostname + '/suhyup/printmade2/server_files/form.txt';
// 설치 화면 이미지 경로
...
var PMCOMPANYFORM = '/SUHYUP/';

function PTM_Ready(){
	if(dmj("input[ptmchecked*='true']").length > 0){
		dmj("input[ptmchecked*='true']").attr('checked','true');
	}
	if(dmj("tbody[style*='block']").length > 0){
		dmj("tbody[style*='block']").css('display','table-row-group');
	}
}
...
function PTMPrintWebPage( options )
{
  ...
  if ( options == undefined )
	...
	//Other options
	GPrintmade.option.SetCompanyform( PMCOMPANYFORM );
	...
	GPrintmade.option.SetFitMode(PMFITMODE);
...
//dmj(frame.document.body).append('<div id = "ptm_down" align = center style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.jpg\');z-index:5001;display:block"></div>');
	if (dmj('.contWrap', frame.document.body).length > 0) {
		dmj('<div id = "ptm_down" align = center style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.jpg\');z-index:5001;display:block"></div>').insertBefore(dmj('.contWrap', frame.document.body));
	} else {
		dmj(frame.document.body).append('<div id = "ptm_down" align = center style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.jpg\');z-index:5001;display:block"></div>');
	}
...
*/
	...
	GetParameterString : function() {
		...
		this._AddParameter('PAGEMARGIN', "10,10,10,20");
		...
		this._AddParameter('FITMODE', this._fitmode);
...
//		}		
		...
		// 크로스 도메인이 발생하여, 다음과 같이 처리하기로함. 
		if(dmj("#iframe_fund").length > 0){
...
/* 		this._insertID(framewin); */
		...
		if(unique_id == 1){
			result = result + '<head><script src=\"'+SCRIPT_PATH+'\"></script><script> window.onload = function(){ PTM_Ready(); } </script>' + result_head + '</head>';
		}else{
