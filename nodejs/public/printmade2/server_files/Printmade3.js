...
var PTMBridge;
var SERVER_PATH = location.protocol + "//" + location.hostname +  '/solution/printmade3';
		...
		Run : function(option){
			...
			if(this.checkInstall()){
				...
				if(this.Browser == "MSIE"){
					...
					if(ieVersion <= 5 && isPTM != -1){
						ptmversion = navigator.userAgent.match(/printmade=\d\d*.\d\d*.\d\d*.\d\d*/gi);
						if(ptmversion.toString().indexOf(PTMDonutVersion) !== -1){
					...
					return false;
				}else if(this.Browser == "Firefox" || this.Browser == "MSIE" || this.Browser == "Safari" ){
		...
		if (unique_id == 1)
		{
			...
			if(GPrintmade.isMac){
				...
				if(GPrintmade.isFirefox3 == true || GPrintmade.isFirefoxOver3 == true  || GPrintmade.isSafari == true || GPrintmade.isSafariOver51 == true){
					...
					var param = "FRAMEID:"+frameid+"|ELEMENTID:"+elementid+"|ALIGNMENT:left|CUTRECT:0,0,0,0|FORMURL:"+formurl+"|PAGEWIDTH:"+pagewidth;
					var ieVersion = "";
					var ptmversion = "";
					var isPTM = navigator.userAgent.indexOf("printmade");
					try{
					...
					if(ieVersion <= 5 && isPTM != -1){
						...
						location.href="printmade25://"+param;	
					}else{
						Donut.RunPrintmade(param);
						PTM_RemoveDialog();
					}
