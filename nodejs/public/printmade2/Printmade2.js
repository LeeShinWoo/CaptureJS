...
// 라이센스 코드
var LICENSE_CODE = '0Fo0lYiKrrxKuqT3+h8NCdaRx13hoTFbbEoa7I9d4eNVJ+OJ3ihP4bwFHQBi0bqD';
// 공통 경로
//var SERVER_PATH = location.protocol + '//' + location.hostname + '/printmade2';
//var SCRIPT_PATH = location.protocol + '//' + location.hostname + '/printmade2/server_files/ptmscript.js';
var SERVER_PATH = 'https://cardimage.shinhancard.com/img/email/printmade2';
var SCRIPT_PATH = 'http://nagosoft.com/install/test/sh/160226/ptmscript.js';
 ...
 * 
 **********************************************************************************************/
var VALIDATECLASS_MSG = unescape('JavaScript%uAC00%20%uC81C%uB300%uB85C%20%uB85C%uB4DC%uB418%uC9C0%20%uC54A%uC558%uC2B5%uB2C8%uB2E4.\n');
var UNDEFINED_WEBBROWSER_MSG = unescape('%uD604%uC7AC%20%uC0AC%uC6A9%uD558%uACE0%20%uC788%uB294%20%uC6F9%uBE0C%uB77C%uC6B0%uC800%uB294%20%uC9C0%uC6D0%uD558%uC9C0%20%uC54A%uC2B5%uB2C8%uB2E4.');
var UNDEFINED_OS_MSG = unescape('%uD604%uC7AC%20%uC0AC%uC6A9%uD558%uACE0%20%uC788%uB294%20OS%uB294%20%uC9C0%uC6D0%uD558%uC9C0%20%uC54A%uC2B5%uB2C8%uB2E4.');
var INSTALL_DIV_MSG = unescape('%uC774%20%uD504%uB85C%uADF8%uB7A8%uC740%20%uC6F9%20%uD398%uC774%uC9C0%20%uC778%uC1C4%uB97C%20%uC704%uD55C%20%uC194%uB8E8%uC158%uC785%uB2C8%uB2E4.');
var INSTALL_LINUX_MSG = unescape('%uC774%20%uD504%uB85C%uADF8%uB7A8%uC740%20%uC6F9%20%uD398%uC774%uC9C0%20%uC778%uC1C4%uB97C%20%uC704%uD55C%20%uC194%uB8E8%uC158%uC785%uB2C8%uB2E4.');

var UPDATE_ERROR_MSG = unescape('Printmade%202%20%uC5C5%uB370%uC774%uD2B8%20%uACFC%uC815%uC5D0%uC11C%20%uC624%uB958%uAC00%20%uC0DD%uACBC%uC2B5%uB2C8%uB2E4.%20%uC0C8%20%uBC84%uC804%uC744%20%uC124%uCE58%uD569%uB2C8%uB2E4.');
  ...
  // 아래의 options.frame_name에서 에러남.
	...
	if ( options.frame_index != undefined ) {
		...
		if ( typeof frameobject == 'string') {
			alert(unescape('PTMPrintWebPage%20%uD568%uC218%uC758%20%5Bframe_index%5D%uAC00%20%uC62C%uBC14%uB978%20%uD504%uB808%uC784%uC744%20%uAC00%uB9AC%uD0A4%uACE0%20%uC788%uC9C0%20%uC54A%uC2B5%uB2C8%uB2E4.\n%uC138%uBD80%uC624%uB958%20%3A%20') + frameobject);
			return;
		} else {
			targetframe = frameobject;
	...
	if ( dmj('frameset',targetframe.document)[0] != undefined ) {
		alert(unescape('PTMPrintWebPage%20%uD568%uC218%uC758%20%5Bframe_name%5D%uC758%20frame%uC740%20frameset%uC774%20%uC544%uB2CC%20body%uB97C%20%uAC00%uC9C0%uACE0%20%uC788%uC5B4%uC57C%20%uD569%uB2C8%uB2E4.'));
 		return;
	...
	if ( options.elementid != undefined ) {
		if ( dmj(options.elementid, targetframe.document).length == 0 ) {
			alert(unescape('PTMPrintWebPage%20%uD568%uC218%uC758%20elementid%uC758%20Element%uB97C%20%uCC3E%uC744%20%uC218%20%uC5C6%uC2B5%uB2C8%uB2E4.'));
			return;
		}
...
function PTM_LoadDialog(){
		...
		try{
			...
			dmj(document.body).append("<div id='ptm-loading' ptm-loading-remove='true' style='position:fixed;z-index:10001;top:"+(document.documentElement.clientHeight-65)/2+"px;left:"+(document.documentElement.clientWidth-240)/2+"px'><img src='"+SERVER_PATH+"/server_files/imgs/load.png'></div>");
...
function PTM_BeforePrinting( options, targetframe ) {
	...
	if(dmj("input[type*='checkbox']").length > 0){
		dmj("input[type*='checkbox']").attr("ptmchecked","false");
	  dmj("input[type*='checkbox']:checked").attr("ptmchecked","true");
	}
}
...
function PTM_ShinhanPrint( setCode ){ //신한은행 인쇄 함수
  var couponMsg = unescape('%uC778%uC1C4%uD560%20%uCFE0%uD3F0%uC774%20%uC5C6%uC2B5%uB2C8%uB2E4.\n%uC778%uC1C4%uD560%20%uCFE0%uD3F0%uC744%20%uC120%uD0DD%uD574%20%uC8FC%uC2ED%uC2DC%uC624');
  PTMVALUE = setCode;
  //font edit
  dmj("table").each(function(){
    dmj(this).attr("ptmfontsize",dmj(this).css("font-size"));
  })
  switch(setCode){
    case 'common' : { //사용안함.
      PTMPrintWebPage({elementid:'#printarea'});
    }
    case 'A_A_printList01' : { //법인(210,213,220,393,449,453)
      PTMPrintWebPage({elementid:'#printarea', objectcapture:'false'});
      break;
    }
    
    case 'B_A_email01' : { //개인 신용 탭1(291,293,388)
      PTMPrintWebPage({elementid:'#email01', objectcapture:'false'});
      break;
    }
    case 'B_A_email02' : { //개인 신용 탭2(291,293,388)

      PTMPrintWebPage({elementid:'#email02', objectcapture:'false'});
      break;
    }
    case 'B_A_email03' : { //개인 신용 탭3(291,293,388)
      PTMPrintWebPage({elementid:'#email03', objectcapture:'false'});
      break;
    }
    case 'B_A_email_check' : { //개인 신용 쿠폰 선택(291,293,388)
      var cnt = dmj("#email04 .check input:checked").length;//쿠폰 객체
      if(cnt < 1) { alert(couponMsg); return; }
      
      PTMPrintWebPage({elementid:'.myCoupon', objectcapture:'false'});
      break;
    }
    case 'B_A_email04' : { //개인 신용 쿠폰 전체 탭4(291,293,388)
      PTMPrintWebPage({elementid:'#email04', objectcapture:'false'});
      break;
    }
    case 'B_A_email05' : { //개인 신용 탭5(291,293,388)
      PTMPrintWebPage({elementid:'#email05', objectcapture:'false'});
      break;
    }
    case 'B_A_email05_N' : { //개인 신용 탭5(291,293,388) 2015.02.02
      PTMPrintWebPage({elementid:'#email04', objectcapture:'false'});
      break;
    }
    case 'B_A_email05_C' : { //체크카드
      PTMPrintWebPage({elementid:'#email03', objectcapture:'false'});
      break;
    }    
    case 'B_A_email06' : { //개인 신용 간편인쇄(291,293,388)
      PTMPrintWebPage({elementid:'#email01', objectcapture:'false'});
      break;
    }
    case 'B_B_list01' : { //체크,신용 탭1(24,259,263,325,389,390,392)
      PTMPrintWebPage({elementid:'#print_tab1', objectcapture:'false'});
      break;
    }
    case 'B_B_list02' : { //체크카드 탭2(24,259,263,325,389,390,392)
      PTMPrintWebPage({elementid:'#print_tab2_title', objectcapture:'false'});
      break;
    }
    case 'B_B_list05' : { //체크카드 탭3(24,259,263,325,389,392)
      PTMPrintWebPage({elementid:'#print_tab5', objectcapture:'false'});
      break;
    }
    case 'B_B_list06' : { //체크카드 청구서(36)
      PTMPrintWebPage({elementid:'#print_top', objectcapture:'false'});
      break;
    }
    case 'B_B_coupon01' : { //탭4 쿠폰 전체(325,389)
      PTMPrintWebPage({elementid:'#print_tab4', objectcapture:'false'});
      break;
    }
    case 'B_B_coupon02' : { //탭3 쿠폰 전체(390)
      PTMPrintWebPage({elementid:'#print_tab3', objectcapture:'false'});
      break;
    }
    case 'B_B_tab011' : { //탭1 간편인쇄(325,389,390)
      PTMPrintWebPage({elementid:'#print_tab1', objectcapture:'false'});
      break;
    }
    case 'B_D_check01' : { //체크론(337)
      PTMPrintWebPage({elementid:'#print_tab1', objectcapture:'false'});
      break;
    }
    case 'B_D_check02' : { //체크론 간편내역서(337)
      PTMPrintWebPage({elementid:'#print_tab1_1', objectcapture:'false'});
      break;
    }
    case 'B_D_list02' : { //대출,론 명세 탭1(289,292,326,391,405)
      PTMPrintWebPage({elementid:'#print_tab1', objectcapture:'false'});
      break;
    }
    case 'B_D_tab01' : { //신용카 영문 탭1(232,303,323,324,451)
      PTMPrintWebPage({elementid:'#print_tab1', objectcapture:'false'});
      break;
    }
    case 'B_D_tab011' : { //영문 신용,체크 간단명세서(323,324,451)
      PTMPrintWebPage({elementid:'#print_tab1', objectcapture:'false'});
      break;
    }
    ...
    case 'B_D_tab02' : { //신용카드 청구서 영문 탭2(303)
       PTMPrintWebPage({elementid:'#print_tab2_title', objectcapture:'false'});
      break;
    }
    case 'B_D_tab022' : { //신용 영문 탭2(323,324,451)
      PTMPrintWebPage({elementid:'#print_tab2_title', objectcapture:'false'});
      break;
    }
    
    case 'B_D_coupon01' : { //쿠폰 탭 전체(289,292,326,391,405)
       PTMPrintWebPage({elementid:'#print_tab2', objectcapture:'false'});
      break;
    }
    case 'B_D_coupon02' : { //쿠폰 선택 공통(289,292,326,303,325,389,390,391,405)
      var cnt = dmj("#print_cou input:checked").length;//쿠폰 객체
      if(cnt < 1) {
        alert(couponMsg); return;
      }
      PTMPrintWebPage({elementid:'#print_cou', objectcapture:'false'});
      break;
    }
    case 'B_D_coupon03' : { //신용 영문 쿠폰 전체(303)
      PTMPrintWebPage({elementid:'#print_tab3', objectcapture:'false'});
      break;
    }
    case 'B_D_info' : { //대출,체크론 인포(289,292,326,337,391,405)
      PTMPrintWebPage({elementid:'#email05', objectcapture:'false'});
      break;
    }
    case 'B_D_info2' : { //대출,체크론 인포2(289,292,326,337,391,405)
      PTMPrintWebPage({elementid:'#email02', objectcapture:'false'});
      break;
    }
    case 'B_D_page' : { //론분리 간단명세서(292)
      PTMPrintWebPage({elementid:'#email01', objectcapture:'false'});
      break;
    }
    case 'B_E_auto' : { //오토리스 탭1(243,256)
      PTMPrintWebPage({elementid:'#customer', objectcapture:'false'});
      break;
    }
    case 'B_E_auto01' : { //오토리스 탭2(243,256)
      PTMPrintWebPage({elementid:'#printarea2', objectcapture:'false'});
      break;
    }
    case 'B_F_page01' : { //공통 (26,35,122,123,139,213,453,212,331,450)
      PTMPrintWebPage({elementid:'#printarea', objectcapture:'false'});
      break;
    }
    case 'B_F_page02' : { //입금 안내문(302)
      PTMPrintWebPage({elementid:'#printarea', objectcapture:'false', cutrect:'0,1,0,1'});
      break;
    }
    case 'B_D_email' : { //일반대출 이용대금()
      if(dmj("#email02").css('display') == 'none'){
        PTMPrintWebPage({elementid:'#email01', objectcapture:'false'});
      } else {
        PTMPrintWebPage({elementid:'#email02', objectcapture:'false'});
      }
...
function PTM_DownloadDialog() {
	...
	dmj("body").append( '<div id = "ptm_down" style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.png\');z-index:5001;display:block"></div>' );
	...
	$down_div.append('<div style="height:140px"></div>');
	$down_div.append('<div style="font-family:\'NanumGothic\';font-weight:bold;padding-left: 138px;font-size:9pt;text-align:left;width:270px;">'+INSTALL_DIV_MSG+'</div>');

	var download_url = '';

	if (GPrintmade.isWindows == true) {
		download_url = WIN_DOWNLOAD_URL;
		$down_div.append('<div id="ptmdownbtn" align="center"><a href = "' + download_url + '" id = "ptm_download" ><img src = "' + INSTALL_IMG_URL + 'download.png" style = "border:0"></a></div>');			
	}
		...
		$down_div.append('<div id="ptmdownbtn" align="center"><a href = "' + fedora_url + '" id = "ptm_fedora_download" '+installmessage+'><img src = "' + INSTALL_IMG_URL + 'download_fedora.png" style = "border:0"></a></div>');
		dmj("#ptmdownbtn").append('&nbsp;&nbsp;<a href = "' + ubuntu_url + '" id = "ptm_ubuntu_download" '+installmessage+'><img src = "' + INSTALL_IMG_URL + 'download_ubuntu.png" style = "border:0"></a>');		                
	}
	dmj("#ptmdownbtn").css("padding-top","40px");
	dmj("#ptmdownbtn").append('&nbsp;&nbsp;<a href = "#" id = "ptm_cancel"><img src = "' + INSTALL_IMG_URL + 'cancel.png" style = "border:0"></a>');
	...
	/*
		dmj("#ptm_down").css('top',(dmj(window).scrollTop() + (dmj(window).height()-295)/2));
		dmj("#ptm_down").css('left',(dmj(window).scrollLeft() + (dmj(window).width()-455)/2));
		dmj("#ptm_bg").css('width',dmj(window).scrollLeft() + dmj(window).width());
		dmj("#ptm_bg").css('height',dmj(window).scrollTop() + dmj(window).height());*/
	});
...
function GetPrintmadeVersion() {
	...
	try {
		...
		return unescape('Printmade%202%20%uBC84%uC804%20%uD655%uC778%20%uACFC%uC815%uC5D0%uC11C%20%uC624%uB958%uAC00%20%uC0DD%uACBC%uC2B5%uB2C8%uB2E4.');
	}
...
Soo.Extend(Soo.Util, {
	...
	FindFrameWithIndex : function ( start_frame, index ) {
		...
		for ( var i = 0; i < index_arr.length; i++ ) {
			if ( frm.frames.length == 0 )
				return unescape('%uD574%uB2F9%20%uC704%uCE58%uC5D0%20%uD504%uB808%uC784%uC774%20%uC874%uC7AC%uD558%uC9C0%20%uC54A%uC2B5%uB2C8%uB2E4.');
			...
			if ( this.CheckCrossDomain(frm) == true ) {
				return unescape('%uD574%uB2F9%20%uD504%uB808%uC784%uC740%20%uD06C%uB85C%uC2A4%20%uB3C4%uBA54%uC778%uC785%uB2C8%uB2E4.');
			}
		}
...
//		this._frame = pframe;
		...
		dmj(frame.document.body).append('<div id = "down" align = center style="position:absolute;top:' + download_div_top + 'px;left:' + download_div_left + 'px;width:'+dialog_width+'px;height:'+dialog_height+'px;background-image:url(\'' + INSTALL_IMG_URL + 'setup.png\');z-index:5001;display:block"></div>');
		var down_div = frame.document.getElementById('down');
		var $down_div = dmj(down_div);
		$down_div.append('<div style="height:130px"></div>');
		$down_div.append('<div><p>'+INSTALL_DIV_MSG+'</p></div>');

		var download_url = '';

		if (GPrintmade.isWindows == true) {
			download_url = WIN_DOWNLOAD_URL;
			$down_div.append('<div id="ptmdownbtn"><a href = "' + download_url + '" id = "ptm_download" ><img src = "' + INSTALL_IMG_URL + 'download.png" style = "border:0"></a></div>');			
		}
		else if (GPrintmade.isMac == true) {
			download_url = MAC_DOWNLOAD_URL;
			$down_div.append('<div id="ptmdownbtn"><a href = "' + download_url + '" id = "ptm_download" ><img src = "' + INSTALL_IMG_URL + 'download.png" style = "border:0"></a></div>');
    		...
    		$down_div.append('<div id="ptmdownbtn"><a href = "' + fedora_url + '" id = "ptm_fedora_download" '+installmessage+'><img src = "' + INSTALL_IMG_URL + 'download_fedora.png" style = "border:0"></a>&nbsp;');
    		$down_div.append('<a href = "' + ubuntu_url + '" id = "ptm_ubuntu_download" '+installmessage+'><img src = "' + INSTALL_IMG_URL + 'download_ubuntu.png" style = "border:0"></a>&nbsp;</div>');		                
		}
	...
	Run:function(target){
		//브라우저 종류를 검색한다.
		 ...
		 if(this._CheckMoblie() == true){
				...
				alert(unescape('%uD504%uB9B0%uD2B8%uBA54%uC774%uB4DC%uB294%20%uBAA8%uBC14%uC77C%20%uAE30%uAE30%uC5D0%uC11C%20%uC9C0%uC6D0%uD558%uC9C0%20%uC54A%uC2B5%uB2C8%uB2E4.'));
			//	window.print();
				return;
		 }
...
/* 		this._insertID(framewin); */
		...
		//result = '<!DOCTYPE html><html ' + html_attributes + '>';
		if(unique_id==1){
			//result = result + '<head>' + result_head + '</head>'; // 내장스크립트 취소
			result = result + '<head><script>var gprinttype = \"'+ PTMVALUE +'\";</script><script src="'+SCRIPT_PATH+'"></script>' + result_head +'</head>'; // 내장스크립트
		}else{
...
//	result = result + this._ResizeBody();
     ...
     */
		...
		dmj(obj).find('img').each(function(){
		   ...
		   */
			if ((new String(dmj(this).attr('src'))).indexOf(current_protocol) == -1 && (new String(dmj(this).attr('src'))).indexOf('http://') && (new String(dmj(this).attr('src'))).indexOf('https://') == -1) {
		...
		dmj(obj).find('link').each(function(){
			...
			if ((new String(dmj(this).attr('href'))).indexOf(current_protocol) == -1 && (new String(dmj(this).attr('href'))).indexOf('http://') && (new String(dmj(this).attr('href'))).indexOf('https://') == -1) {
		...
		dmj(obj).find('*[background]').each(function(){
			if ((new String(dmj(this).attr('background'))).indexOf(current_protocol) == -1 && (new String(dmj(this).attr('background'))).indexOf('http://') && (new String(dmj(this).attr('background'))).indexOf('https://') == -1) {
...
//		클래스가 실질적으로 복사되는 코드
		...
		if(typeof cutsize == "object"){
			max_table_row_size = cutsize[1]; // 1 이상으로 해야햠!
			max_table_row_size_first = cutsize[0];
		}else{
			max_table_row_size = cutsize; // 1 이상으로 해야햠!
		}
