...
var SCRIPTURL = SERVER_PATH + '/server_files/ptmscript3.js';
/**********************************************************************************************
 *
 * Global Options
 *
...
/*
 var lastBreakTop = 0;
 var pageWidth = 1100;
 function createPageBreak(parent_Node){
 	tableBreak.count = 0;
 	lastBreakTop = parent_Node.offsetTop;
 	nodeCheck(parent_Node);
  }
 function nodeCheck(parent_Node){
 	console.log(parent_Node);
 	var children = parent_Node.childNodes;
 	for(var i=0; i<children.length; i++){
 		if(!isNaN(children[i].offsetTop)){
 			if(children[i].offsetHeight > pageWidth){
 				var cha = 0;
 				var table_children = children[i].childNodes;
 				for(var j=0; j<table_children.length; j++){
 					if(table_children[j].nodeName == "TBODY"){
 						var tr = table_children[j].childNodes;
 						cha = tr[3].offsetTop - tr[1].offsetTop;
 					}
 				}
 				tableBreak(children[i].offsetTop,tableBreak.count,cha);
 				tableBreak.count++;
 			}
 			else if(children[i].hasChildNodes()){
 				nodeCheck(children[i]);
 			}
 			if(children[i].offsetTop - lastBreakTop > pageWidth){
 				children[i].outerHTML = children[i].outerHTML + "<pagebreak></pagebreak>";
 				lastBreakTop = children[i].offsetTop;
 			}
 		}
 	}
 }

 function tableBreak(nodeTop,count,tr_Height){
 	var table = document.getElementsByTagName("Table");
 	var table_count = 0;
 	var tableStart = nodeTop;
 	for(var i=0;i<table.length; i++){
 		var table_tr = table[i].getElementsByTagName("tr");
 		if(table[i].offsetHeight > pageWidth && table_count == count){
 			var table_thead = table[i].getElementsByTagName("thead");
 			for(var j=0;j<table_tr.length; j++){
 				tableStart += tr_Height;
 				if(tableStart - lastBreakTop > pageWidth){
 					var addBar = table_tr[j].outerHTML + "<tr><td><pagebreak></pagebreak></tr></td>" +table_tr[0].outerHTML;
 					table_tr[j].outerHTML = addBar;
 					lastBreakTop = tableStart;
 					tableStart += tr_Height;
 				}
 			}
 		}else if(table[i].offsetHeight > pageWidth){
 			table_count++;
 		}
 	}
 }
...
/*
	if(options.elementid != undefined){
		var printArea = document.getElementById(options.elementid.split('#')[1]);
		createPageBreak(printArea);
	}
	*/
	ptmcnt = 0;
	ptmstarted = false;
	...
	PTMMainController.__init__();
	GPrintmade = PTMMainController;
	if(GPrintmade == null)
		return;
...
Soo.Extend(Soo.Util, {
	FindFrame : function (frame, id) {
		if (PTMChecker.checkCrossDomain(frame) == true)
		{
			return null;
		}
...
var PTMCaptureController = {
	...
	_Capture : function(){
		PTMCaptureEngine.__init__(this._plugin,this._targetframe,this._imagefilepath);
		g_tmpcaptureclass = PTMCaptureEngine;
	...
	Initialize:function() {
		...
		if (GPrintmade.option.GetCaptureEnabled() == true) {
			if(this.plugincapture == null){
				PTMCaptureController.__init__(this._plugin, this.target);
				this.plugincapture = PTMCaptureController;
			}
